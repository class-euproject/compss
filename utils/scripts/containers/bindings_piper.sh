#!/bin/sh
  ...
  if [ "${virtualEnvironment}" != "null" -a "${propagateVirtualEnvironment}" = "true" ]; then
  ...
  if [ "$tracing" -gt 0 ]; then
    ...
    echo $( $(cat "${baseConfigFile}") | sed s/{{PATH}}/"${escapedConfigPath}"/g ) > "${workerConfigFile}"
  ...
  while [ "${stop_received}" = "false" ]; do
    ...
    case "${cmd_tag}" in
      ...
      "START_WORKER")
        ...
        if [ "${binding}" = "PYTHON" -a "${mpiWorker}" = "true" ]; then
          # delimiter example: "python -u"
          delimiter="${pythonInterpreter} -u"
          #bindingExecutable="${workerCMD%%"$delimiter"*}"
          bindingExecutable="$(echo ${workerCMD} | cut -d"$delimiter" -f1)"
          #bindingArgs=${workerCMD#*"$delimiter"}
          bindingArgs=$(echo ${workerCMD} | cut -d"$delimiter" -f2-)
          ...
          if [ "${tracing}" -eq -1 ]; then # scorep
            ...
            bindingExecutable="$(echo ${workerCMD} | cut -d"$delimiter" -f1)$delimiter"
          fi
      ...
      "GET_ALIVE")
        ...
        if [ "${line}" != "GET_ALIVE" ]; then
          pids="${line}"
          alive_processes=$(ps h -o pid,stat ${pids} | awk '$2 != "Z" && $1 != "PID" {print $1}')
